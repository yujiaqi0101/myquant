...
def setup_track_parser(parser: argparse.ArgumentParser) -> None:
    ...
    p_lb.add_argument(
        "--view", default="all",
        choices=["all", "strategies", "experiments"],
        help="显示视图: all(双表) / strategies(策略元数据) / experiments(最近实验)",
    )
    p_lb.add_argument("--tag", default=None, help="按标签筛选策略")
    ...
    # rebuild-best-perf
    p_rbp = sub.add_parser("rebuild-best-perf", help="重建策略最佳表现")
    p_rbp.add_argument("--strategy", default=None, help="指定策略ID（不指定则重建全部）")
    p_rbp.add_argument("--dry-run", action="store_true", help="仅显示待更新项，不实际写入")
...
def track_quantlab_command(args: argparse.Namespace) -> None:
    ...
    if args.track_action == "list":
        ...
        _rebuild_best_perf(args)

    else:
        print("请指定 track 子动作: list / show / leaderboard / search / delete / rebuild-best-perf")


def _show_leaderboard(args, repo) -> None:
    """leaderboard 双表显示：策略元数据 + 最近实验"""
    view = getattr(args, 'view', 'all')
    tag = getattr(args, 'tag', None)
    db_path = _get_db_path()

    # 表1: 策略最佳表现 (strategy_best_perf)
    if view in ('all', 'strategies'):
        from src.data.database import DatabaseManager
        db = DatabaseManager(db_path)
        with db.get_connection() as conn:
            sql = '''
                SELECT bp.strategy_id, bp.version, bp.best_sharpe, bp.best_return,
                       bp.best_max_dd, bp.best_experiment_id, bp.last_updated
                FROM strategy_best_perf bp
            '''
            params = []
            if tag:
                sql += '''
                    INNER JOIN strategy_tag st ON bp.strategy_id = st.strategy_id
                    WHERE st.tag = ?
                '''
                params.append(tag)
            sql += ' ORDER BY bp.best_sharpe DESC'
            df_bp = pd.read_sql_query(sql, conn, params=params)

        if df_bp.empty:
            print("\n[策略最佳表现] 暂无数据（运行 rebuild-best-perf 生成）")
        else:
            print(f"\n=== 策略最佳表现 (共 {len(df_bp)} 个) ===")
            print(df_bp.to_string(index=False))

    # 表2: 最近实验 (quantlab_experiments + quantlab_results)
    if view in ('all', 'experiments'):
        df_exp = repo.leaderboard(sort_by=args.sort_by, top=args.top)
        if df_exp.empty:
            print("\n[最近实验] 暂无数据")
        else:
            print(f"\n=== 最近实验排行榜 (sort_by={args.sort_by}, top={args.top}) ===")
            cols = [
                "id", "name", "strategy",
                "sharpe", "total_return", "max_drawdown",
            ]
            cols = [c for c in cols if c in df_exp.columns]
            print(df_exp[cols].to_string(index=False))


def _rebuild_best_perf(args) -> None:
    """重建策略最佳表现"""
    from src.data.database import DatabaseManager
    from src.quantlab_adapters.best_perf_updater import BestPerfUpdater

    db_path = _get_db_path()
    db = DatabaseManager(db_path)
    updater = BestPerfUpdater(db)
    ...
    if dry_run:
        ...
        stale = updater.list_stale_best_perf()
        print(f"\n=== Best Perf 干运行 ===")
        print(f"  缺失的策略: {missing if missing else '无'}")
        print(f"  过期的策略: {stale if stale else '无'}")
        return

    if strategy:
        result = updater.update_strategy_best_perf(strategy)
        if result:
            print(f"策略 '{strategy}' 最佳表现已更新: Sharpe={result['best_sharpe']:.4f}")
        else:
            print(f"策略 '{strategy}' 无实验数据")
    else:
        count = updater.rebuild_all_best_perf()
        print(f"全表重建完成: 更新了 {count} 个策略的最佳表现")
